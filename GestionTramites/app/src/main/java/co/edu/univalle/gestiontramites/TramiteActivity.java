...
public class TramiteActivity extends AppCompatActivity {
    ...
    private void cargarSpinners() {
        ...
        // Cargar requisitos
        listaRequisitos = conexionBD.obtenerRequisitos(); // Debes tener este método en ConexionBD
        List<String> nombresRequisitos = new ArrayList<>();
        for (Requisito r : listaRequisitos) {
            nombresRequisitos.add(r.getDescripcionRequisito());
        }
        if (nombresRequisitos.isEmpty()) {
            nombresRequisitos.add("Sin requisitos");
        }
    ...
    private void guardarTramite() {
        ...
        tramite.setFrecuencia(""); // Ya no se usa campo frecuencia
        ...
        if (res > 0) {
            // Guardar requisito seleccionado si no es "Sin requisitos"
            ...
            if (!listaRequisitos.isEmpty() && posReq >= 0 && posReq < listaRequisitos.size()) {
                ...
                // Asociar el requisito al trámite si es necesario (puedes crear una relación en la BD)
                // Ejemplo: actualizar el idTramite del requisito seleccionado
                reqSeleccionado.setIdTramite((int) res);
