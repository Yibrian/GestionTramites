...
public class TramiteAdapter extends RecyclerView.Adapter<TramiteAdapter.TramiteViewHolder> {
    ...
    public void onBindViewHolder(@NonNull TramiteViewHolder holder, int position) {
        ...
        // Mostrar descripción del requisito asociado
        ConexionBD conexionBD = ConexionBD.getInstance(context);
        List<co.edu.univalle.gestiontramites.model.Requisito> requisitos = conexionBD.obtenerRequisitosPorTramite(t.getIdTramite());
        if (!requisitos.isEmpty()) {
            holder.tvRequisitos.setText("Requisito: " + requisitos.get(0).getDescripcionRequisito());
        } else {
            holder.tvRequisitos.setText("Requisito: Sin requisito");
        }
    ...
    public void actualizarNombres(List<String> nuevasCiudades, List<String> nuevosTipos) {
        this.nombresCiudades = nuevasCiudades;
        this.nombresTipos = nuevosTipos;
        notifyDataSetChanged();
    }
